...
export class NamingStrategy implements NamingStrategyInterface {
  ...
  columnName(
    ...
    if (embeddedPrefixes.length)
      return (
        underscore(embeddedPrefixes.join("_")) +
        (customName ? underscore(customName) : underscore(propertyName))
      );

    return underscore(customName || propertyName);
  }
  ...
  joinTableColumnName(
    ...
    return underscore(`${tableName}_${columnName || propertyName}`);
  }
